...
import dev.luisghtz.myaichat.image.validators.ImageValidator;
...
class ImageValidatorTest {
  ...
  void testIsValid_InvalidContentType_ReturnsFalse() {
    ...
    when(mockContext.buildConstraintViolationWithTemplate("File is not an image")).thenReturn(mockViolationBuilder);
    when(mockViolationBuilder.addConstraintViolation()).thenReturn(mockContext);
    ...
    assertFalse(result);
    verify(mockContext).disableDefaultConstraintViolation();
    verify(mockContext).buildConstraintViolationWithTemplate("File is not an image");
    verify(mockViolationBuilder).addConstraintViolation();
  }
